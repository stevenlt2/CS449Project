...
class BoardTest {
	...
	public void testRecordAndReplay() {
		...
		board.endRecording();
		board.makeSMove(2, 2); 
		board.simpleGameEndCheck(); //Blue wins.
		board.endRecording();
		assertEquals("", board.getWinner(), 'B');
		assertEquals("", board.getStoredBoardSize(), 5);
		board.resizeBoard(board.getStoredBoardSize());
		board.replay();
		board.simpleGameEndCheck();
		board.replay();
		board.simpleGameEndCheck();
	...
	public void testDeleteRecord() {
		...
		assertEquals("", board.recRow.empty(), true);
		assertEquals("", board.recCol.empty(), true);
		assertEquals("", board.recType.empty(), true);
		assertEquals("", board.repRow.empty(), true);
		assertEquals("", board.repCol.empty(), true);
		assertEquals("", board.repType.empty(), true);
	}
}
